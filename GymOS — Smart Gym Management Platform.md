...
*(background business case originally documented as "FitCore" — see Open Questions §31.1)*
...
| :---- | :---- |
| **Alternate / Background Reference** | (background business case originally documented as "FitCore" — see Open Questions §31.1) |
...
| :---- | :---- | :---- | :---- |
| v0.1 | May 2025 | Founder / Product Owner | FitCore Business Proposal drafted (business case, market analysis, pricing) |
| v2.0 | Undated (supplied 2026\) | Founder / Product Owner | GymOS Role Function List, Business Rules & Logic, Project Scope & Workflow Map, Role Workflows & Feature Guide, and Workflow & Process Flow documents produced |
| v1.0 | August 2, 2026 | Business Analyst (Claude) | Consolidated all supplied source documents into a single SRS; simplified role model to SuperAdmin / Gym Owner / PT Trainer / Gym Member for current build phase |

# **2\. Executive Summary**

## **2.1 Overview**

GymOS is a cloud-based, multi-tenant Software-as-a-Service (SaaS) platform for gym operators, unifying three core user groups — the Gym Member, the Personal Trainer, and the Gym Owner — into a single system spanning a web application (Owner Dashboard and Trainer Portal) and a mobile application (Member app, and on-floor Trainer functions). A fourth role, SuperAdmin, sits above these as the platform-level system owner (see Section 7).

This document consolidates the following source materials, supplied by the Founder / Product Owner, into a single, structured Software Requirements Specification: the GymOS Role Function List (v2.0), GymOS Business Rules & Logic (v2.0), GymOS Project Scope & Workflow Map (v2.0), GymOS Role Workflows & Feature Guide (v2.0), the GymOS Workflow & Process Flow document (v2.0), and the earlier FitCore Software Project Proposal (May 2025), which documents a related business case for a comparable Sri Lanka-focused gym management product. Where these documents agree, this SRS presents a single confirmed requirement. Where they conflict or one covers ground the other does not, this SRS flags the discrepancy explicitly (see Section 31, Open Questions) rather than silently choosing one interpretation.

## **2.2 Purpose of the System**

GymOS exists to replace manual, paper-based, spreadsheet-based, and informal chat-based (e.g., WhatsApp) gym administration with a single automated system that: (a) lets members hire and train with a personal trainer directly, with the trainer managing that client's sessions and plans, (b) automates the entire Personal Training session lifecycle from booking through billing, (c) gives gym owners real-time financial and operational visibility, (d) helps gyms retain members through automated, data-driven engagement, and (e) — in later phases — adds biometric gym-access control, AI-driven business intelligence, and wearable-derived personalisation on top of that operational foundation. Phase 1 authentication (both client applications) is username/password; biometric access control is Phase 3 (see Section 5.0).

## **2.3 Business Problem Being Addressed**

● Personal Training session attendance, cancellations, and no-shows are tracked manually or informally, causing revenue leakage and pay-calculation disputes.

● Gym owners lack real-time visibility into revenue, trainer performance, and member retention risk.

● Member engagement (workout plans, meal plans, progress tracking) is delivered ad hoc (e.g., via screenshots or messaging apps) with no structured, auditable record.

● Membership renewals and PT session-pack purchases are tracked manually, leading to missed renewals and lost revenue.

● \[Background context from the FitCore business proposal\] Existing manual processes are common among small-to-medium gyms, and foreign gym-management software is often priced in foreign currency without local payment-gateway or local-language support — this context is included as background only and its direct applicability to the GymOS product scope is noted as an Open Question (Section 31).

## **2.4 Proposed Solution**

GymOS delivers this through ten functional modules (see Section 8, System Overview): Access Control, PT Session Tracker, Owner Dashboard, Trainer Portal, Member App, Member Retention Engine, Gamification & Community, Wearable Integration, AI Agent, and Trainer Marketplace — released across three confirmed development phases (Phase 1: MVP; Phase 2: Engagement Platform; Phase 3: AI & Scale).

## **2.5 Expected Business Value**

● Elimination of manual PT-session and billing tracking, reducing revenue leakage from missed billing, no-shows, and pay-calculation errors.

● Real-time financial and operational visibility for gym owners without manual report-building.

● Improved member retention through automated churn scoring and re-engagement sequences.

● Reduced administrative burden on trainers through automated pay calculation and integrated plan/client management.

● \[Recommendation / background\] Consistent, professional member experience intended to differentiate against informal, manual gym administration — a value proposition emphasised in the FitCore business case for the broader market opportunity.
...
● Progress tracking (photos, measurements, weight) is informal with no structured record.

● Admins have no consolidated view of trainer performance, session counts, or member retention.

## **3.3 Reason for Developing the System**

GymOS is being developed to digitise and unify these fragmented, manual processes into a single platform purpose-built for the three core gym roles (Member, Trainer, Owner/Admin), automating the operational and financial mechanics that are currently handled manually or not at all.

## **3.4 Business Context**

GymOS is designed as a multi-tenant SaaS product, billed to gyms on a recurring subscription basis across tiers (tier names differ between source documents — see Open Questions §31.9). The FitCore business proposal frames the initial target market as the Sri Lankan fitness industry specifically, citing an estimated 600–800 registered private gyms nationally; this market context is retained here as background only, since it originates from a separate business-case document rather than the GymOS functional specification set.

# **4\. Objectives**

## **4.1 Primary Objectives**

● OBJ-01 — Let members hire a personal trainer directly and let trainers manage that private client relationship (sessions, workout/meal plans) — Phase 1's Private Trainer Hire.
...
● OBJ-05 — Improve member retention through automated churn scoring, re-engagement sequences, and a structured onboarding journey (Phase 2).

● OBJ-06 — Increase member engagement through gamification (streaks, challenges, badges, opt-in leaderboard) (Phase 2).

● OBJ-07 — Enable public trainer discovery and client acquisition through an in-app Trainer Marketplace, building on Phase 1's Private Trainer Hire (Phase 2).

● OBJ-08 — Provide gym Owners with AI-driven business intelligence — natural-language Q\&A, automated weekly management accounts, anomaly detection, and forecasting (Phase 3).

● OBJ-09 — Integrate wearable device data to personalise member coaching and flag trainer-actionable signals such as overtraining (Phase 3).

● OBJ-10 — Digitise gym-door entry and attendance tracking via biometric access control (face ID / fingerprint), eliminating manual/paper-based check-in (Phase 3).

# **5\. Scope**

## **5.0 Phase Re-Scoping (Current Revision)**

This revision re-scopes three areas based on direct product direction from the Founder/Product Owner, superseding the original phase mapping implied by the GymOS source documents where the two conflict:

● Biometric access control (face ID / fingerprint gym-door entry) is removed from Phase 1 and moved to Phase 3\. Phase 1 authentication is username/password only, for both the Web Dashboard and the Mobile App. See Section 9 (FR-002, FR-010–FR-013) and Section 12 (Business Rules, Membership & Access Rules / Access Control Rules categories) for how this affects existing biometric-dependent requirements and rules.

● Private Trainer Hire — a simplified, non-public "hire a personal trainer" flow — is added to Phase 1\. A member can hire a specific trainer directly; the trainer then manages that member as a private client, tracking their sessions and sending workout/meal plans separately from other clients. This is distinct from the full, public Trainer Marketplace (browsing, filtering, ratings, 48-hour response SLA), which remains Phase 2\. See Section 9, new module W, FR-082–FR-085.

● Exactly two client applications are in scope: one Web Dashboard and one Mobile App (not three or more). The Web Dashboard is built first; the Mobile App is built second. "Trainer Portal" in this document refers to the Trainer-facing views inside the Web Dashboard, not a separate product. See Section 22.9 for full build-sequencing guidance.

 

## **5.1 In Scope**

The following modules are confirmed in scope, mapped to their development phase after the re-scoping above:

| Module | Phase | Primary Users | Summary |
| :---- | :---- | :---- | :---- |
| Authentication (Username/Password) | Phase 1 | Members, Trainers, Gym Owner, SuperAdmin | Username/password account creation and login for both client applications. Replaces biometric login for Phase 1 (see 5.0). |
| PT Session Tracker | Phase 1 | Members, Trainers, Owners | QR-based session lifecycle (start/end); automated billing; cancellation policy enforcement; no-show management. |
| Private Trainer Hire | Phase 1 | Members, Trainers | Simplified, non-public hire flow — member selects and hires a specific trainer; trainer manages the resulting private client relationship, including per-client workout/meal plans. |
| Owner Dashboard | Phase 1 | Gym Owner, SuperAdmin | Real-time revenue, sessions, trainer performance, pack alerts, weekly financial charts. Part of the Web Dashboard application. |
| Trainer Portal (views within the Web Dashboard \+ Mobile App) | Phase 1 | Personal Trainers | Session management, client list, workout/meal plans, earnings. Management tasks live in the Web Dashboard; on-floor session actions live in the Mobile App. |
| Member App | Phase 1 | Members | PT sessions, plans, hire-a-trainer flow, billing. Delivered in the Mobile App. |
| Member Retention Engine | Phase 2 | Owners, AI Agent | Churn risk scoring, automated re-engagement sequences, 30-day onboarding journey. |
| Gamification & Community | Phase 2 | Members | Attendance streaks, monthly challenges, opt-in leaderboard, milestone badges, rewards. |
| Trainer Marketplace (public) | Phase 2 | Members, Trainers | Public trainer discovery and profile directory, PT interest requests, ratings, virtual session support — the full version of Phase 1's Private Trainer Hire. |
| Access Control (Biometric) | Phase 3 | Members, Gym Owner | Biometric face/fingerprint gym-door entry; every check-in feeds attendance analytics and retention scoring. Moved from Phase 1 per 5.0. |
| Wearable Integration | Phase 3 | Members, Trainers | Apple Health, Google Fit, Fitbit, Garmin — biometric data into training and AI insights. |
| AI Agent | Phase 3 | Owners, Members | Management accounts, anomaly detection, dynamic pricing, forecasting, member AI coaching chat. |

 

## **5.2 Out of Scope**

● Granular platform-admin tooling beyond what SuperAdmin needs for day-to-day operation (e.g., a dedicated multi-tenant billing UI, or a self-serve subscription-management flow for other gyms) — not detailed enough in source material to specify now; SuperAdmin's core access is defined in Section 7\.

● In-app Point-of-Sale (POS) for retail/supplement sales, equipment maintenance logging, and AI-generated workout-adjustment suggestions — these appear only in the FitCore business proposal's "Elite Plan" feature table and are not present anywhere in the GymOS Role Function List, Business Rules, or Workflow documents. Treated as out of scope for GymOS unless confirmed otherwise (see Open Questions §31.9).

● Multi-branch / multi-location management — mentioned only in the FitCore proposal ("Elite tier"); not described in any GymOS functional document. Out of scope pending confirmation.

● Sinhala and Tamil language localisation — mentioned only as a FitCore "Phase 2" business roadmap item, not part of the GymOS functional specification set.

● Biometric hardware procurement, kiosk installation, and any physical site-survey process — deferred to Phase 3 along with the rest of Access Control (see 5.0). No hardware dependency exists for Phase 1 or Phase 2\.

● Formal compliance certification (e.g., PCI-DSS, GDPR, or any local data-protection regime) — not confirmed as a project requirement in any source document; see Open Questions §31.7.

 

## **5.3 Future Scope**

● Biometric access control (Phase 3, see 5.0) — face ID / fingerprint gym-door entry, once hardware is procured and a vendor confirmed (Open Questions Q-11).

● Full public Trainer Marketplace (Phase 2\) — discovery, filtering, ratings, and virtual sessions, layered on top of Phase 1's Private Trainer Hire.

● Full AI Agent capability set (dynamic pricing, demand forecasting, cash-flow forecasting) beyond initial anomaly detection and weekly reporting — Phase 3, as scoped.

● Expanded wearable provider support and deeper wearable-driven personalisation (Phase 3).

● \[From FitCore background\] Multi-language interface support, multi-branch management, and an in-app retail/POS module, should these be confirmed as extensions of the GymOS product line rather than a separate FitCore product.

● \[To Be Confirmed\] Custom report builder and priority-support SLA tiers, as referenced only in the FitCore proposal's top-tier plan.

# **6\. Project Parties**

The parties below are relevant context for development — who defines requirements, who operates the system, and which external providers the system integrates with.
...
The following matrix summarises access at a module level. "Full" indicates complete create/read/update capability within the module; "View" indicates read-only access; "—" indicates no access. Items marked \[TBC\] indicate the exact boundary still needs to be confirmed with the Product Owner (see Section 31).
...
● Private Trainer Hire (Phase 1\) establishes a trainer-client relationship that the PT Session Tracker, plan assignment, and earnings modules all key off — once a member hires a trainer, all subsequent sessions, plans, and billing for that pair flow through the same mechanisms already described for those modules.

● Workout/meal plans created by a Trainer become immediately visible to that specific client in the Member App, and — where enabled — update the AI Coach's response context (Phase 3).

● Gamification events (Phase 2: streaks, milestones, challenges) are driven by the same session-completion events that feed retention and billing, rather than being a separate data stream.

● The AI Agent (Phase 3\) reads across session, revenue, expense, and attendance data on a read-only basis (BR-AI-005) to generate Owner-facing reports and alerts, and does not independently trigger member-facing or billing actions.

● Biometric Access Control (Phase 3\) will feed the Owner's live occupancy panel and the Member Retention Engine's churn scoring once implemented, the same way check-in data does in the source material's original design — this is deferred, not removed (see Section 5.0).

## **8.3 High-Level System Workflow (Phase 1\)**

At a summary level, for the current Phase 1 scope: a Gym Owner configures the gym (plans, pay rates, policies) via the Web Dashboard → registers members and trainers (username/password accounts) → a member hires a trainer directly (Private Trainer Hire) → the trainer assigns that client a workout/meal plan and they book/attend PT sessions via the Mobile App's QR lifecycle → the system automatically bills the member, pays the trainer, and records revenue/expenses → the Owner reviews real-time dashboards on the Web Dashboard. Phase 2 layers in retention automation, gamification, and the public Trainer Marketplace; Phase 3 adds biometric access control, wearables, and the AI Agent. Detailed step-by-step workflows for every major process are documented in Section 13\.

## **8.4 Major Modules**

See Section 5.1 (In Scope) for the complete module list with phase mapping. Phase 1 modules are: Authentication, PT Session Tracker, Private Trainer Hire, Owner Dashboard, Trainer Portal, and Member App. Phase 2 adds: Member Retention Engine, Gamification & Community, and the public Trainer Marketplace. Phase 3 adds: Access Control (Biometric), Wearable Integration, and AI Agent.

## **8.5 Client Applications & Role Coverage**

Exactly two client applications are in scope — not three or more. Section 22.9 covers technical build-sequencing guidance in full; this subsection covers which roles use which application.
...
*"Trainer Portal" elsewhere in this document refers to the Trainer-facing views inside the Web Dashboard (management/planning tasks) plus the Trainer-facing views inside the Mobile App (on-floor session execution) — it is not a third, separate application. A Personal Trainer uses one account across both apps.*

# **9\. Functional Requirements**

This section documents all functional requirements derived from the GymOS Role Function List, Business Rules & Logic, and Workflow documents. Requirements are grouped by module. Closely related, low-level functions describing a single cohesive user capability (e.g., "view" and "edit" of the same profile) have been consolidated into one requirement to avoid unnecessary fragmentation, per instruction; each requirement remains independently identifiable, testable, and traceable via its Requirement ID.

Priority definitions: Must Have \= required for the phase in which the module is scoped (see Section 5.1); Should Have \= important but not release-blocking; Could Have \= valuable if time/resources permit, and in several cases corresponds to explicit Phase 2/3 scope.
...
### **FR-006 — Connect/Disconnect Wearable Device**

| Requirement ID | FR-006 |
| :---- | :---- |
| **Module** | A. Member Account & Profile |
| **Priority** | Could Have (Phase 3\) |
| **Description** | The system shall allow a member to connect or disconnect a supported wearable device (Apple Health, Google Fit, Fitbit, Garmin, WHOOP) to sync recovery, heart-rate, and activity data. |
| **User / Actor** | Member |
| **Preconditions** | • Member is logged in |
| **Trigger** | Member selects 'Connect Wearable' in settings. |
| **Main Workflow** | 1\. Member selects a wearable provider 2\. System initiates the OAuth/authorisation flow for that provider \[integration mechanism TBD — see Section 16\] 3\. On success, system links the wearable account and begins syncing data |
| **Alternative / Exception Flows** | • Authorisation fails or is declined: system shows an error and leaves the member unconnected |
| **Business Rules** | — |
| **Expected Outcome** | Wearable data becomes available to feed the member's wellness score, recovery indicator, and the trainer's overtraining alerts. |
...
### **FR-009 — Automated 30-Day Onboarding Journey**

| Requirement ID | FR-009 |
| :---- | :---- |
| **Module** | A. Member Account & Profile |
| **Priority** | Must Have |
| **Description** | Upon membership activation, the system shall automatically run a 30-day onboarding sequence of welcome content and milestone prompts (Day 1, 3, 7, 14, 21, 30), logging completion or skip status for each step. |
| **User / Actor** | System (automated); Member (recipient) |
| **Preconditions** | • Membership has just been activated |
| **Trigger** | Membership activation event. |
| **Main Workflow** | 1\. Day 1: welcome message \+ orientation checklist sent 2\. Day 3: if no PT session booked, a prompt to book a free fitness assessment is sent 3\. Day 7: first-week milestone notification \+ workout plan reminder 4\. Day 14: progress check-in prompt (body measurement/photo) 5\. Day 21: community/challenge invitation \+ leaderboard introduction 6\. Day 30: month-completion badge \+ personalised progress summary |
| **Alternative / Exception Flows** | • Member skips a step: step is recorded as 'skipped'; sequence continues uninterrupted; Owner's onboarding funnel report reflects the drop-off (BR-ONB007) |
| **Business Rules** | ONB001, ONB002, ONB003, ONB004, ONB005, ONB006, ONB007 |
| **Expected Outcome** | Every new member receives a structured, automated first-month engagement journey, and Owners can see completion/drop-off analytics. |
...
## **B. Access Control (Biometric Check-in) — Phase 3**

### **FR-010 — Biometric Gym Entry (Face ID / Fingerprint)**

| Requirement ID | FR-010 |
| :---- | :---- |
| **Module** | B. Access Control (Biometric Check-in) — Phase 3 |
| **Priority** | Could Have (Phase 3 — deferred from Phase 1, see Section 5.0) |
| **Description** | The system shall authenticate a member at the gym entrance via face or fingerprint biometric scan and open the gate only if all of: (1) a biometric match exists, (2) membership is active and unexpired, and (3) the account is not suspended. Not part of Phase 1 — Phase 1 uses username/password authentication only (FR-002); this requirement applies once Phase 3 biometric hardware is introduced. |
| **User / Actor** | Member; Biometric Device |
| **Preconditions** | • Member has completed biometric enrolment (Phase 3\) |
| **Trigger** | Member presents biometric at the entrance kiosk. |
| **Main Workflow** | 1\. Device captures the scan 2\. System checks for a biometric match 3\. System checks membership status (active/expired) 4\. System checks suspension flag 5\. If all checks pass: gate opens and a check-in record is created (member ID, timestamp, method, result \= granted) |
| **Alternative / Exception Flows** | • No biometric match: access denied; unrecognised attempt logged; Gym Owner alerted (BR-MEM004) • Membership expired: access denied; renewal prompt sent to member; Gym Owner alerted (BR-MEM002) • Account suspended: access denied; reason flagged in access log; Gym Owner alerted (BR-MEM003) • 3 consecutive failed scans on the same biometric: member is blocked for 5 minutes and an incident is logged; Gym Owner is alerted (BR-ACC005) |
| **Business Rules** | MEM001, MEM002, MEM003, MEM004, MEM005, ACC002, ACC005 |
| **Expected Outcome** | Only members in good standing are granted physical access, and every attempt (successful or not) is logged. (Phase 3 only.) |

 

### **FR-011 — Manual Access Override**

| Requirement ID | FR-011 |
| :---- | :---- |
| **Module** | B. Access Control (Biometric Check-in) — Phase 3 |
| **Priority** | Could Have (Phase 3 — deferred from Phase 1, see Section 5.0) |
| **Description** | The system shall allow the Gym Owner (or SuperAdmin) to manually override a denied biometric access attempt, logging the override with user ID, timestamp, member ID, and reason. Not applicable until Phase 3 biometric access control exists. |
| **User / Actor** | Gym Owner; SuperAdmin |
| **Preconditions** | • An access denial has occurred or is anticipated (e.g., hardware fault) — Phase 3 only |
| **Trigger** | User selects 'Override Access' for a member. |
| **Main Workflow** | 1\. Staff selects the member and enters an override reason 2\. System records the override as a valid check-in, tagged as manually overridden |
| **Alternative / Exception Flows** | None specified in source material |
| **Business Rules** | ACC003 |
| **Expected Outcome** | Member gains entry despite a biometric/system denial, with full audit traceability. (Phase 3 only.) |

 

## **C. PT Session Booking & Lifecycle**

### **FR-013 — View Personal Session Attendance History & Streak**

| Requirement ID | FR-013 |
| :---- | :---- |
| **Module** | C. PT Session Booking & Lifecycle |
| **Priority** | Should Have |
| **Description** | The system shall allow a member to view their own PT session attendance history (date, time, trainer, status) and current/best attendance streak. In Phase 1/2, the streak is calculated from PT session attendance (QR start/end), since biometric gym-door check-in does not exist until Phase 3 — see the note below and Open Questions Q-14. |
| **User / Actor** | Member |
| **Preconditions** | • Member is logged in |
| **Trigger** | Member opens Session History screen. |
| **Main Workflow** | 1\. System retrieves the member's PT session log 2\. System displays entries chronologically with streak counter and milestones, calculated from session attendance |
| **Alternative / Exception Flows** | • Phase 3: once biometric gym-door check-in exists, \[To Be Confirmed\] whether the streak/history definition expands to include general gym visits in addition to PT sessions, or remains PT-session-based — see Open Questions Q-14. |
| **Business Rules** | GAM001, GAM002 |
| **Expected Outcome** | Member can review their PT session attendance record and streak status. |
...
### **FR-025 — View Wellness Score & Recovery Indicator (Wearable-Derived)**

| Requirement ID | FR-025 |
| :---- | :---- |
| **Module** | E. Training & Nutrition Plans |
| **Priority** | Could Have (Phase 3\) |
| **Description** | The system shall calculate and display a weekly wellness score and a pre-training recovery indicator derived from connected wearable data (heart rate, sleep, activity). |
| **User / Actor** | Member |
| **Preconditions** | • Member has connected a wearable device (FR-006) |
| **Trigger** | Member opens the Wellness/Recovery screen, or before a scheduled training day. |
| **Main Workflow** | 1\. System aggregates wearable metrics 2\. System computes and displays the wellness score / recovery indicator |
| **Alternative / Exception Flows** | • No wearable connected: feature is unavailable / hidden |
| **Business Rules** | — |
| **Expected Outcome** | Member (and trainer, via TRN008) gain insight into readiness for training. |

 

## **F. AI Coaching Assistant**

### **FR-026 — AI Coach Chat (Member-Facing)**

| Requirement ID | FR-026 |
| :---- | :---- |
| **Module** | F. AI Coaching Assistant |
| **Priority** | Could Have (Phase 3\) |
| **Description** | The system shall provide an in-app AI chat assistant, available only when enabled per-client by the assigned trainer, constrained to answer using the member's assigned plan content and to decline and escalate any injury/medical topic. |
| **User / Actor** | Member; AI System; Personal Trainer |
| **Preconditions** | • Trainer has enabled AI Coach for this member (FR-035) |
| **Trigger** | Member opens the AI Coach chat tab. |
| **Main Workflow** | 1\. Chat interface loads with the member's current plan injected as AI context and recent message history loaded 2\. Member sends a message or responds to an automated daily check-in prompt 3\. System scans the message for injury/pain/medical keywords 4\. If no medical keyword: AI generates a plan-constrained response; conversation is stored and visible to the trainer 5\. If a medical keyword is detected: AI declines to answer; conversation is flagged for trainer review; trainer is notified |
| **Alternative / Exception Flows** | • AI Coach not enabled for this member: chat screen shows a locked state directing the member to ask their trainer to activate it |
| **Business Rules** | TRN006, TRN007 |
| **Expected Outcome** | Members receive plan-aware coaching support without the AI providing unsupervised medical guidance. |

 

### **FR-027 — AI Daily Check-in Prompts & Motivational Messages**

| Requirement ID | FR-027 |
| :---- | :---- |
| **Module** | F. AI Coaching Assistant |
| **Priority** | Could Have (Phase 3\) |
| **Description** | The system shall send automated daily check-in prompts and motivational/goal-reminder messages from the AI Coach on non-PT-session days, where enabled. |
| **User / Actor** | System (automated); Member |
| **Preconditions** | • AI Coach is enabled for the member |
| **Trigger** | Scheduled daily job on non-PT days. |
| **Main Workflow** | 1\. System sends a check-in prompt or motivational message via the AI Coach chat |
| **Alternative / Exception Flows** | None specified in source material |
| **Business Rules** | — |
| **Expected Outcome** | Member engagement with training is reinforced on days without a scheduled PT session. |
...
### **FR-029 — Browse Trainer Marketplace & Submit PT Interest Request**

| Requirement ID | FR-029 |
| :---- | :---- |
| **Module** | G. Trainer Communication |
| **Priority** | Should Have (Phase 2\) |
| **Description** | The system shall allow a member to browse a directory of trainers (filterable by specialisation/availability) and submit a PT interest request with a goals note to a chosen trainer, who has 48 hours to respond. |
| **User / Actor** | Member; Personal Trainer |
| **Preconditions** | • Trainer Marketplace is enabled by the Owner and the trainer has opted in |
| **Trigger** | Member browses the marketplace and selects a trainer. |
| **Main Workflow** | 1\. Member views trainer profiles (name, specialisations, ratings, availability) 2\. Member submits an interest request with a note on goals 3\. Request is delivered to the trainer; 48-hour response timer starts 4\. Trainer accepts (proposes an introductory session) or declines |
| **Alternative / Exception Flows** | • No trainer response within 48 hours: request expires; member is informed; Owner is notified (BR-MKT003) |
| **Business Rules** | MKT001, MKT003 |
| **Expected Outcome** | A structured trainer-discovery and matching process exists independent of Owner intervention. |
...
### **FR-031 — Attendance Streak Tracking**

| Requirement ID | FR-031 |
| :---- | :---- |
| **Module** | H. Gamification & Community |
| **Priority** | Should Have (Phase 2\) |
| **Description** | The system shall track a weekly attendance streak per member, incrementing when a configurable weekly visit target (default 3 check-ins/week) is met, and resetting with a recovery nudge when missed. |
| **User / Actor** | Member; System (automated) |
| **Preconditions** | • Member has an active membership |
| **Trigger** | Member check-in event. |
| **Main Workflow** | 1\. System checks the member's check-in count for the current week against the target 2\. Target met: streak counter increments; milestone check runs 3\. Target missed at week end: streak resets to 0; a recovery-nudge notification is sent |
| **Alternative / Exception Flows** | None specified in source material |
| **Business Rules** | GAM001, GAM002 |
| **Expected Outcome** | Consistent gym attendance is measured and reinforced through streaks. |

 

### **FR-032 — Milestone Badges & Rewards**

| Requirement ID | FR-032 |
| :---- | :---- |
| **Module** | H. Gamification & Community |
| **Priority** | Should Have (Phase 2\) |
| **Description** | The system shall automatically award milestone badges (first visit, 10th visit, 100th visit, 30-day streak, pack completion, challenge completion) and trigger any Owner-configured reward. |
| **User / Actor** | System (automated); Member |
| **Preconditions** | • A qualifying milestone event occurs |
| **Trigger** | Relevant check-in, streak, or pack-completion event. |
| **Main Workflow** | 1\. System detects the milestone condition is met 2\. Badge is awarded and a notification is sent to the member 3\. If the Owner has configured a reward for that milestone, the reward is triggered |
| **Alternative / Exception Flows** | None specified in source material |
| **Business Rules** | GAM003, GAM004, GAM005, GAM006, GAM009 |
| **Expected Outcome** | Members receive recognisable, automated positive reinforcement for engagement milestones. |

 

### **FR-033 — Monthly Challenges & Opt-in Leaderboard**

| Requirement ID | FR-033 |
| :---- | :---- |
| **Module** | H. Gamification & Community |
| **Priority** | Could Have (Phase 2\) |
| **Description** | The system shall allow members to join Owner-created challenges and track progress, and shall provide an opt-in leaderboard (ranked by check-ins, PT sessions, challenge completions) that defaults to OFF for new members. |
| **User / Actor** | Member; Gym Owner |
| **Preconditions** | • Owner has created at least one active challenge |
| **Trigger** | Member views/joins a challenge, or opts into the leaderboard. |
| **Main Workflow** | 1\. Member joins an active challenge 2\. System tracks and displays progress toward the challenge goal 3\. On goal completion: completion badge awarded, reward triggered, leaderboard rank updated 4\. Member may opt in/out of the leaderboard at any time |
| **Alternative / Exception Flows** | None specified in source material |
| **Business Rules** | GAM007, GAM008 |
| **Expected Outcome** | Community engagement features exist without compromising member privacy by default (opt-in only). |
...
### **FR-035 — Enable/Disable AI Coach Per Client**

| Requirement ID | FR-035 |
| :---- | :---- |
| **Module** | I. Trainer Account & Profile |
| **Priority** | Could Have (Phase 3\) |
| **Description** | The system shall allow a trainer to enable or disable the AI Coaching Assistant on an individual, per-client basis. |
| **User / Actor** | Personal Trainer |
| **Preconditions** | • Trainer has an assigned client |
| **Trigger** | Trainer toggles the AI Coach switch on a client profile. |
| **Main Workflow** | 1\. Trainer toggles AI Coach ON/OFF for the selected client 2\. System updates the client's AI Coach availability accordingly; client is notified when enabled |
| **Alternative / Exception Flows** | None specified in source material |
| **Business Rules** | TRN006 |
| **Expected Outcome** | AI coaching access is explicitly trainer-controlled per client, not global. |
...
### **FR-040 — Overtraining Alert (Wearable-Derived)**

| Requirement ID | FR-040 |
| :---- | :---- |
| **Module** | J. Trainer Session Management |
| **Priority** | Could Have (Phase 3\) |
| **Description** | The system shall alert a trainer when a client's connected wearable indicates 3 or more consecutive days of high fatigue, suggesting a review of upcoming session intensity. |
| **User / Actor** | System (automated); Personal Trainer |
| **Preconditions** | • Client has a connected wearable with sufficient data history |
| **Trigger** | Wearable data ingestion detects the fatigue-threshold condition. |
| **Main Workflow** | 1\. System evaluates rolling wearable fatigue signals daily 2\. Threshold breached: alert generated and delivered to the assigned trainer |
| **Alternative / Exception Flows** | None specified in source material |
| **Business Rules** | TRN008 |
| **Expected Outcome** | Trainers receive early warning of potential client overtraining. |
...
### **FR-043 — Accept/Decline PT Interest Requests & Propose Introductory Session**

| Requirement ID | FR-043 |
| :---- | :---- |
| **Module** | K. Trainer Client Management |
| **Priority** | Should Have (Phase 2\) |
| **Description** | The system shall allow a trainer to accept or decline a marketplace PT interest request within 48 hours, and, on acceptance, propose an introductory session. |
| **User / Actor** | Personal Trainer |
| **Preconditions** | • A PT interest request exists for the trainer |
| **Trigger** | Member submits an interest request (FR-029). |
| **Main Workflow** | 1\. Trainer reviews the request (member name, goals note) 2\. Trainer accepts (proposes an intro session) or declines within 48 hours |
| **Alternative / Exception Flows** | • No response within 48h: request expires; Owner notified (BR-MKT003) |
| **Business Rules** | MKT003 |
| **Expected Outcome** | New client relationships are formally initiated and tracked. |
...
## **M. Trainer Marketplace**

### **FR-047 — Public Trainer Profile & Marketplace Visibility**

| Requirement ID | FR-047 |
| :---- | :---- |
| **Module** | M. Trainer Marketplace |
| **Priority** | Should Have (Phase 2\) |
| **Description** | The system shall display a trainer's public profile (as members see it) in the Trainer Marketplace only when Owner-level marketplace is enabled AND the trainer has individually opted in. |
| **User / Actor** | Personal Trainer; Gym Owner |
| **Preconditions** | • Owner has enabled marketplace at the gym level |
| **Trigger** | Trainer toggles their own marketplace opt-in. |
| **Main Workflow** | 1\. Trainer opts in 2\. Profile becomes visible to all members in the app directory |
| **Alternative / Exception Flows** | • Owner disables marketplace gym-wide: no trainer profiles are shown regardless of individual opt-in |
| **Business Rules** | MKT001 |
| **Expected Outcome** | Marketplace visibility is governed by a two-level (gym \+ trainer) opt-in control. |

 

### **FR-048 — Virtual Session Scheduling & Billing**

| Requirement ID | FR-048 |
| :---- | :---- |
| **Module** | M. Trainer Marketplace |
| **Priority** | Could Have (Phase 2\) |
| **Description** | The system shall allow a trainer who has opted into virtual sessions to schedule, start, and complete a virtual (video/audio) session, billed identically to an in-person session (including off-shift rate rules). |
| **User / Actor** | Personal Trainer; Member |
| **Preconditions** | • Trainer has opted into offering virtual sessions |
| **Trigger** | Trainer or member schedules a virtual session. |
| **Main Workflow** | 1\. Session is scheduled as type 'Virtual' 2\. Trainer marks the session started, then completed 3\. System deducts the client's pack and records revenue as with an in-person session 4\. If outside shift hours, the off-shift rate is applied automatically |
| **Alternative / Exception Flows** | None specified in source material |
| **Business Rules** | MKT004, MKT005 |
| **Expected Outcome** | Virtual sessions are fully integrated into the standard billing and payroll pipeline. |
...
### **FR-053 — Configure Gamification Settings**

| Requirement ID | FR-053 |
| :---- | :---- |
| **Module** | N. Gym Setup & Configuration |
| **Priority** | Should Have (Phase 2\) |
| **Description** | The system shall allow the Owner to configure the weekly streak target, available challenge types, and milestone rewards. |
| **User / Actor** | Gym Owner |
| **Preconditions** | • Owner is authenticated |
| **Trigger** | Owner opens Gamification settings. |
| **Main Workflow** | 1\. Owner sets streak target, defines challenges, assigns rewards to milestones 2\. System saves; changes reflected in member app on next session refresh |
| **Alternative / Exception Flows** | None specified in source material |
| **Business Rules** | — |
| **Expected Outcome** | Gamification mechanics are tunable per gym rather than fixed. |

 

### **FR-054 — Configure Re-engagement Message Templates**

| Requirement ID | FR-054 |
| :---- | :---- |
| **Module** | N. Gym Setup & Configuration |
| **Priority** | Should Have (Phase 2\) |
| **Description** | The system shall allow the Owner to configure the content of automated at-risk (Day 1/3/7) and lapsed (Day 1/7) re-engagement messages; GymOS controls the timing/trigger logic while the Owner controls tone/content/offers. |
| **User / Actor** | Gym Owner |
| **Preconditions** | • Owner is authenticated |
| **Trigger** | Owner opens Re-engagement Templates settings. |
| **Main Workflow** | 1\. Owner edits message templates per churn band/day 2\. System saves and applies templates to future automated sends |
| **Alternative / Exception Flows** | None specified in source material |
| **Business Rules** | CHU002, CHU003 |
| **Expected Outcome** | Retention messaging reflects the gym's own voice while remaining system-automated. |
...
### **FR-059 — View Onboarding Funnel & Send Manual Re-engagement Message**

| Requirement ID | FR-059 |
| :---- | :---- |
| **Module** | O. Member Management |
| **Priority** | Should Have (Phase 2\) |
| **Description** | The system shall display per-member onboarding journey status (steps completed, drop-off point) and allow the Gym Owner to send a manual re-engagement message to any at-risk or lapsed member. |
| **User / Actor** | Gym Owner |
| **Preconditions** | • Member has an onboarding record or churn score |
| **Trigger** | Gym Owner opens the Retention/Onboarding panel. |
| **Main Workflow** | 1\. System displays onboarding funnel and at-risk/lapsed member lists 2\. Gym Owner selects a member and sends a message/offer 3\. Action is logged in the re-engagement history |
| **Alternative / Exception Flows** | None specified in source material |
| **Business Rules** | ONB007, CHU002, CHU003 |
| **Expected Outcome** | Owner can intervene manually in addition to fully automated re-engagement sequences. |
...
### **FR-062 — Enable/Disable Trainer Marketplace Visibility (Gym-Level)**

| Requirement ID | FR-062 |
| :---- | :---- |
| **Module** | P. Trainer Management |
| **Priority** | Should Have (Phase 2\) |
| **Description** | The system shall allow the Owner to enable or disable Trainer Marketplace visibility at the gym level, acting as a master switch over individual trainer opt-ins. |
| **User / Actor** | Gym Owner |
| **Preconditions** | • Owner is authenticated |
| **Trigger** | Owner toggles Marketplace setting. |
| **Main Workflow** | 1\. Owner toggles the gym-level marketplace switch 2\. System applies the setting across all trainers immediately |
| **Alternative / Exception Flows** | None specified in source material |
| **Business Rules** | MKT001 |
| **Expected Outcome** | Owner retains centralised control over whether the marketplace feature is active for their gym. |
...
### **FR-072 — Member Retention Heatmap & Churn Prediction Report**

| Requirement ID | FR-072 |
| :---- | :---- |
| **Module** | S. Owner Dashboard & Analytics |
| **Priority** | Should Have (Phase 2\) |
| **Description** | The system shall display a member retention heatmap (attendance frequency colour-coded by churn risk) and a weekly churn-prediction report with suggested intervention actions per member. |
| **User / Actor** | Gym Owner |
| **Preconditions** | • Sufficient attendance history exists for churn scoring |
| **Trigger** | Owner opens the Retention/Analytics panel. |
| **Main Workflow** | 1\. System computes and renders the heatmap and churn report weekly |
| **Alternative / Exception Flows** | None specified in source material |
| **Business Rules** | CHU001-006 |
| **Expected Outcome** | Owner can visually identify at-risk segments of the membership base. |

 

### **FR-073 — Operational Analytics Suite (Onboarding Funnel, Gamification Engagement, Wearable Adoption, Trainer Utilisation, Attendance, Growth)**

| Requirement ID | FR-073 |
| :---- | :---- |
| **Module** | S. Owner Dashboard & Analytics |
| **Priority** | Should Have |
| **Description** | The system shall provide reports covering: onboarding funnel completion/drop-off, gamification engagement statistics, wearable adoption rate, trainer utilisation (sessions vs. availability/idle hours), member attendance patterns (peak hours, average visits), and new-member growth (sign-ups by month, source attribution). |
| **User / Actor** | Gym Owner |
...
| **Business Rules** | — |
| **Expected Outcome** | Owner has a broad operational analytics suite beyond core financial/session reporting. |

 

## **T. AI Agent (Owner Intelligence Layer)**

### **FR-074 — Natural-Language Q\&A Over Gym Data**

| Requirement ID | FR-074 |
| :---- | :---- |
| **Module** | T. AI Agent (Owner Intelligence Layer) |
| **Priority** | Could Have (Phase 3\) |
| **Description** | The system shall provide a chat interface allowing the Owner to ask natural-language questions about financial and operational data; the AI Agent shall respond using only real data retrieved from GymOS and shall not fabricate figures. |
| **User / Actor** | Gym Owner; AI Agent |
| **Preconditions** | • AI Agent module is enabled (subscription tier permitting) |
| **Trigger** | Owner submits a question via the AI chat. |
| **Main Workflow** | 1\. AI retrieves relevant data from the GymOS database 2\. AI composes and returns a data-grounded response 3\. Query and response are logged |
| **Alternative / Exception Flows** | • Data required to answer is unavailable: AI must not hallucinate a figure (BR-AI-006) — exact fallback behaviour \[TBD\] |
| **Business Rules** | AI-005, AI-006 |
| **Expected Outcome** | Owner can query business performance conversationally without building custom reports. |

 

### **FR-075 — Automated Weekly Management Accounts & Anomaly Detection**

| Requirement ID | FR-075 |
| :---- | :---- |
| **Module** | T. AI Agent (Owner Intelligence Layer) |
| **Priority** | Could Have (Phase 3\) |
| **Description** | Every week (default Monday morning, configurable), the system shall automatically generate a P\&L summary and KPI scorecard, run anomaly checks (revenue drop, no-show spike, expense overrun vs. configurable thresholds), and deliver alerts to the Owner via dashboard and email. |
| **User / Actor** | System (automated AI Agent); Gym Owner |
| **Preconditions** | • Sufficient prior-week data exists |
| **Trigger** | Scheduled weekly job (default Monday). |
| **Main Workflow** | 1\. AI retrieves prior week's financial, session, and attendance data 2\. AI generates the P\&L summary and KPI scorecard 3\. AI runs anomaly checks against configurable thresholds 4\. Report and any alerts are delivered to the Owner dashboard and via email |
| **Alternative / Exception Flows** | • No anomalies found: a 'clean report' is delivered with no alerts |
| **Business Rules** | AI-001, AI-002, AI-003, AI-004 |
| **Expected Outcome** | Owner receives a proactive, automated weekly business review without manual report-building. |

 

### **FR-076 — Dynamic Pricing Recommendations & Revenue/Demand Forecasting**

| Requirement ID | FR-076 |
| :---- | :---- |
| **Module** | T. AI Agent (Owner Intelligence Layer) |
| **Priority** | Could Have (Phase 3\) |
| **Description** | The system shall surface AI-generated recommendations for off-peak pricing (based on consistently below-target utilisation time slots), PT demand forecasts, cash-flow forecasts (30/60/90 days), membership renewal risk scoring, and revenue-gap alerts when projected weekly revenue falls below a configurable target. |
| **User / Actor** | AI Agent (automated); Gym Owner |
| **Preconditions** | • Sufficient historical utilisation/financial data exists |
| **Trigger** | Scheduled AI analysis job / Owner views the AI Agent panel. |
| **Main Workflow** | 1\. AI analyses utilisation and financial trends 2\. AI surfaces pricing, forecast, and risk recommendations to the Owner dashboard |
| **Alternative / Exception Flows** | None specified in source material |
| **Business Rules** | FIN006, AI-007 |
| **Expected Outcome** | Owner receives forward-looking, data-driven commercial recommendations. |

 

## **U. Access Control Management**

### **FR-077 — Live Entry/Exit Log & Access Denial Report**

| Requirement ID | FR-077 |
| :---- | :---- |
| **Module** | U. Access Control Management |
| **Priority** | Must Have |
| **Description** | The system shall provide the Owner a real-time live entry/exit log, a full access log (all check-ins and denials with timestamp and method), and an access-denial report (reasons, member names, timestamps). |
| **User / Actor** | Gym Owner |
| **Preconditions** | • Access events have occurred |
| **Trigger** | Owner opens the Access Control panel. |
| **Main Workflow** | 1\. System retrieves and displays access event logs, filterable by outcome/reason |
| **Alternative / Exception Flows** | None specified in source material |
| **Business Rules** | — |
| **Expected Outcome** | Owner has full auditability of physical access activity. |

 

### **FR-078 — Biometric Device Configuration & Member Re-enrolment**

| Requirement ID | FR-078 |
| :---- | :---- |
| **Module** | U. Access Control Management |
| **Priority** | Must Have |
| **Description** | The system shall allow the Owner to configure biometric device settings (enrol new devices, update firmware \[TBD scope\]) and manually initiate re-enrolment of a member's biometric data when scans consistently fail. |
| **User / Actor** | Gym Owner |
| **Preconditions** | • A biometric device is installed and connected |
| **Trigger** | Owner initiates device configuration or member re-enrolment. |
| **Main Workflow** | 1\. Owner selects the device/member and performs the configuration/re-enrolment action 2\. System updates the stored biometric token or device configuration accordingly |
| **Alternative / Exception Flows** | None specified in source material |
| **Business Rules** | ACC004 |
| **Expected Outcome** | Biometric access issues can be resolved by gym staff without vendor/developer escalation for routine cases. |
...
Major functional requirements are expressed below as user stories with testable acceptance criteria. Each story references the Functional Requirement(s) it derives from implicitly via its subject matter (see Section 32, Traceability Matrix, for explicit ID-level mapping).
...
| User Story | As a Member, I want to check in using my fingerprint or face, so that I don't need a physical card and my visit is logged automatically. |
| :---- | :---- |
| **Priority** | Could Have (Phase 3 — deferred, see Section 5.0; Phase 1 uses username/password login only, US-025 covers Phase 1 access) |
| **Acceptance Criteria** | • A successful biometric match with active, unsuspended membership opens the gate and logs a check-in record with timestamp and method • An expired membership or unrecognised biometric denies access and alerts the Gym Owner • 3 consecutive failed scans locks the member out for 5 minutes and logs an incident |
...
| User Story | As a Member, I want to chat with an AI Coach about my plan, so that I can get quick answers without waiting for my trainer to reply. |
| :---- | :---- |
| **Priority** | Could Have (Phase 3\) |
| **Acceptance Criteria** | • The AI Coach tab is only available if my trainer has enabled it for me • Questions about my assigned plan receive a relevant, plan-based answer • Any message referencing injury, pain, or a medical issue is declined by the AI and flagged for my trainer |

 

### **US-009**

| User Story | As a Member, I want to build attendance streaks and earn badges, so that I stay motivated to keep coming back. |
| :---- | :---- |
| **Priority** | Should Have (Phase 2\) |
| **Acceptance Criteria** | • My streak increases when I meet the weekly visit target and resets (with a nudge) if I miss it • Milestone badges (first visit, 10th, 100th, 30-day streak) are awarded automatically • I control whether I appear on the leaderboard, which defaults to off |

 

### **US-010**

| User Story | As a Member, I want to browse trainers and request one that fits my goals, so that I can find the right match without asking the front desk. |
| :---- | :---- |
| **Priority** | Should Have (Phase 2\) |
| **Acceptance Criteria** | • I can view trainer profiles including specialisation, rating, and next available slot • I can submit an interest request with a note about my goals • I am informed of the trainer's response, or told the request expired after 48 hours with no reply |
...
| User Story | As a Trainer, I want to respond to marketplace interest requests from prospective clients, so that I can grow my client roster through the platform. |
| :---- | :---- |
| **Priority** | Should Have (Phase 2\) |
| **Acceptance Criteria** | • I am notified when a member submits an interest request with their name and goals • I can accept (and propose an introductory session) or decline within 48 hours • If I don't respond within 48 hours, the Owner is notified and the request expires |
...
| User Story | As a Gym Owner, I want to see which members are at risk of leaving and be able to reach out, so that I can reduce churn proactively. |
| :---- | :---- |
| **Priority** | Should Have (Phase 2\) |
| **Acceptance Criteria** | • Members are automatically scored 0-100 weekly and banded Active/At-Risk/Lapsed • At-risk and lapsed members trigger automated notification sequences without my involvement • I can send a manual, one-tap message or offer to any flagged member |

 

### **US-022**

| User Story | As a Gym Owner, I want an AI-generated weekly summary of my business with anomaly alerts, so that I don't have to manually build reports every week. |
| :---- | :---- |
| **Priority** | Could Have (Phase 3\) |
| **Acceptance Criteria** | • Every Monday, a P\&L summary and KPI scorecard is generated automatically • Any revenue drop, no-show spike, or expense overrun beyond a configurable threshold is flagged with context • I can ask follow-up questions in plain language and receive answers grounded in real system data |
...
| User Story | As a Gym Owner, I want to manually override access for a member when the biometric scan fails, so that a legitimate member isn't locked out of the gym. |
| :---- | :---- |
| **Priority** | Could Have (Phase 3 — deferred, see Section 5.0) |
| **Acceptance Criteria** | • I can select a member and record an override with a reason • The override is logged with my user ID, timestamp, and the member ID • The override is treated as a valid, auditable check-in |
...
The following use cases detail the major end-to-end system interactions.
...
### **UC-02 — Daily Biometric Gym Access**

| Actor(s) | Member |
| :---- | :---- |
| **Goal** | Gain physical entry to the gym via biometric authentication. |
| **Preconditions** | • Member has previously completed biometric enrolment |
| **Main Flow** | 1\. Member presents face/fingerprint at the entrance kiosk 2\. System verifies biometric match 3\. System verifies membership is active and unexpired 4\. System verifies the account is not suspended 5\. Gate opens and a check-in record is created |
| **Alternative Flow** | • Front desk manually overrides a denial with a logged reason (BR-ACC003) |
| **Exception Flow** | • No biometric match: access denied, incident logged, front desk alerted • Membership expired: access denied, renewal prompt sent, front desk alerted • Account suspended: access denied, reason logged, front desk alerted • 3 consecutive failed scans: member locked out 5 minutes, incident logged |
| **Postconditions** | • A check-in (or denial) record exists; attendance/streak/churn calculations are updated on success. |
...
### **UC-07 — AI Coaching Chat Interaction**

| Actor(s) | Member, AI Agent, Personal Trainer |
| :---- | :---- |
| **Goal** | Provide the member with plan-aware coaching support while safely escalating medical topics. |
| **Preconditions** | • Trainer has enabled AI Coach for this member |
| **Main Flow** | 1\. Member opens the AI Coach chat; plan context and recent history are loaded 2\. Member sends a message or responds to a daily check-in prompt 3\. System scans for injury/pain/medical keywords 4\. If none found: AI responds using plan-constrained context; conversation is logged and visible to the trainer |
| **Alternative Flow** | • Message is a routine check-in prompt response rather than a member-initiated question |
| **Exception Flow** | • Medical keyword detected: AI declines to respond on that topic, flags the conversation, and notifies the trainer, who then responds directly |
| **Postconditions** | • Conversation history is stored and trainer-visible; any flagged topic is routed to human review. |

 

### **UC-08 — Trainer Marketplace Matching**

| Actor(s) | Member, Personal Trainer, Gym Owner |
| :---- | :---- |
| **Goal** | Connect a member with a new trainer through the marketplace directory. |
| **Preconditions** | • Gym-level marketplace is enabled and the trainer has opted in |
| **Main Flow** | 1\. Member browses/filters trainer profiles and submits an interest request with a goals note 2\. Trainer reviews and accepts within 48 hours, proposing an introductory session 3\. Session is confirmed and the standard PT session lifecycle (UC-04) begins 4\. Member may submit a rating/review afterward, subject to Owner moderation |
| **Alternative Flow** | • Trainer declines the request: member is notified |
| **Exception Flow** | • Trainer does not respond within 48 hours: request expires, member is informed, Owner is notified |
| **Postconditions** | • A new trainer-client relationship is established (or the request is closed/expired). |

 

### **UC-09 — Churn Scoring & Automated Re-engagement**

| Actor(s) | System (automated), Gym Owner |
| :---- | :---- |
| **Goal** | Identify at-risk/lapsed members and automatically (or manually) re-engage them. |
| **Preconditions** | • Members have attendance/engagement history |
| **Main Flow** | 1\. Weekly job calculates a 0-100 churn score per member from weighted signals 2\. Members are banded Active (0-39) / At-Risk (40-69) / Lapsed (70-100) 3\. At-Risk and Lapsed bands trigger a scheduled sequence of automated notifications culminating in an Owner dashboard alert 4\. Owner may supplement with a manual, one-tap message or offer |
| **Alternative Flow** | • Member checks in during the sequence: sequence halts, status is restored to Active, score recalculated |
| **Exception Flow** | • Member does not respond within the full sequence: member remains in the current band pending next weekly recalculation |
| **Postconditions** | • Member's churn band and any resulting engagement actions are current and logged. |
...
### **UC-13 — Weekly AI Management Accounts Generation**

| Actor(s) | AI Agent (automated), Gym Owner |
| :---- | :---- |
| **Goal** | Automatically produce a weekly business performance summary with anomaly detection. |
| **Preconditions** | • AI Agent module enabled for the gym's subscription tier |
| **Main Flow** | 1\. Scheduled job retrieves prior week's revenue, expense, session, and attendance data 2\. AI generates a P\&L summary and KPI scorecard 3\. AI runs anomaly checks against configurable thresholds 4\. Report and any alerts are delivered to the dashboard and via email |
| **Alternative Flow** | • Owner asks a natural-language follow-up question after reviewing the report |
| **Exception Flow** | • No anomalies detected: a clean report is delivered with no alerts |
| **Postconditions** | • Owner has an automated weekly performance summary, with or without flagged anomalies. |

 

### **UC-14 — Gamification Engagement Cycle**

| Actor(s) | Member, System (automated) |
| :---- | :---- |
| **Goal** | Track and reward member attendance/engagement automatically. |
| **Preconditions** | • Member has an active membership |
| **Main Flow** | 1\. Member checks in; system checks weekly visit count against the streak target 2\. Streak increments if target met (else resets with a nudge) 3\. System checks for milestone conditions (visit counts, streak length, pack completion, challenge goals) 4\. Any milestone triggers a badge, notification, and Owner-configured reward |
| **Alternative Flow** | • Member has joined an active challenge: challenge progress and leaderboard rank are also updated |
| **Exception Flow** | • \[TBD\] No explicit exception path defined; all outcomes are deterministic based on check-in data |
| **Postconditions** | • Member's streak, badges, and (if opted in) leaderboard position are current. |
...
Every business rule below is transcribed directly from the GymOS Business Rules & Logic document (v2.0). Each rule has been assigned a sequential BR-XXX identifier for traceability, alongside its original source reference ID (e.g., MEM001) as documented by the Product Owner. No business rule has been invented; any rule that appeared implied but unconfirmed is marked accordingly in the relevant Functional Requirement or in Section 31 (Open Questions).

*Note: the current build uses four roles only — SuperAdmin, Gym Owner, Personal Trainer, Gym Member (Section 7). A small number of source rules (e.g., BR-USR002, BR-USR003) reference a delegated "Manager" role that is not part of the current role model; these are retained below for completeness and traceability but are not active requirements until/unless that role is reintroduced (see FR-080).*

*Note: per the Phase re-scoping in Section 5.0, biometric gym-door entry is Phase 3, not Phase 1\. Rules BR-001–BR-005 (MEM001–005) and BR-070–BR-074 (ACC001–005) below describe biometric-dependent behaviour and apply to Phase 3\. Rules BR-006 and BR-007 (MEM006–007, membership expiry/auto-suspension) remain Phase 1 requirements, but in Phase 1 their effect is to block session booking and app features rather than a biometric gate — see FR-012.*

## **1\. Membership & Access Rules**

| BR ID | Source Ref | Rule | Condition | Outcome / Action |
| :---- | :---- | :---- | :---- | :---- |
| BR-001 | MEM001 | Gym entry — active membership | Member scans biometric at door | If membership is active and not expired: gate opens, check-in logged with timestamp and method |
| BR-002 | MEM002 | Gym entry — expired membership | Member scans biometric, membership expired | Access denied, notification sent to member and Gym Owner, gate remains locked |
| BR-003 | MEM003 | Gym entry — suspended account | Member scans biometric, account suspended | Access denied, reason flagged in access log, Gym Owner alerted |
| BR-004 | MEM004 | Biometric not recognised | Scan fails to match any enrolled profile | Access denied, unrecognised attempt logged, Gym Owner override available |
| BR-005 | MEM005 | Check-in logging | Every successful gate open | Check-in record created: member ID, timestamp, method (face/fingerprint), result |
...
## **3\. Churn & Retention Rules**

| BR ID | Source Ref | Rule | Condition | Outcome / Action |
| :---- | :---- | :---- | :---- | :---- |
| BR-019 | CHU001 | Churn score calculation | Runs every 7 days for all members | Score (0-100) calculated from weighted signals: days since last check-in (high), check-in frequency trend (high), PT no-show rate (medium), membership expiry proximity (medium), app engagement (low), onboarding completion (low) |
| BR-020 | CHU002 | At-risk band | Churn score 40-69 | Member status set to At Risk; Day 1 push notification triggered; Day 3 SMS triggered; Day 7 owner dashboard alert with intervention options |
| BR-021 | CHU003 | Lapsed band | Churn score 70-100 | Member status set to Lapsed; immediate owner notification; Day 1 re-engagement campaign; Day 7 AI-generated personalised message |
| BR-022 | CHU004 | Active band | Churn score 0-39 | Member status remains Active; no automated outreach triggered |
| BR-023 | CHU005 | Re-engagement sequence end | Member checks in during a re-engagement sequence | Sequence halted, churn score recalculated, status updated to Active |
| BR-024 | CHU006 | 7-day no check-in flag | Member has not checked in for 7 or more days | Member flagged in owner dashboard retention panel; at-risk sequence initiated if churn score crosses threshold |
...
| BR ID | Source Ref | Rule | Condition | Outcome / Action |
| :---- | :---- | :---- | :---- | :---- |
| BR-025 | GAM001 | Weekly streak increment | Member meets weekly visit target (default: 3 check-ins in a week) | Streak counter incremented; streak milestone checked |
| BR-026 | GAM002 | Weekly streak break | Member fails to meet weekly visit target | Streak counter reset to 0; recovery nudge notification sent |
| BR-027 | GAM003 | Milestone: first visit | Member completes first ever check-in | Welcome badge awarded; notification sent |
| BR-028 | GAM004 | Milestone: 10 visits | Member's 10th lifetime check-in | 10-visit badge awarded |
| BR-029 | GAM005 | Milestone: 100 visits | Member's 100th lifetime check-in | Loyalty badge awarded; owner-configured reward triggered |
| BR-030 | GAM006 | Milestone: 30-day streak | Member maintains weekly target for 30 consecutive days | Streak badge awarded; owner-configured reward triggered |
| BR-031 | GAM007 | Challenge completion | Member meets challenge goal before end date | Completion badge awarded; reward notification sent; leaderboard rank updated |
| BR-032 | GAM008 | Leaderboard opt-in default | New member registered | Leaderboard participation set to OFF by default; member must opt in manually |
| BR-033 | GAM009 | PT pack completion milestone | Member uses final session in a pack | Pack Completion badge awarded; renewal prompt sent immediately |
...
| BR ID | Source Ref | Rule | Condition | Outcome / Action |
| :---- | :---- | :---- | :---- | :---- |
| BR-034 | ONB001 | Day 1 welcome checklist | Membership activated | Welcome message \+ gym orientation checklist sent to member app |
| BR-035 | ONB002 | Day 3 PT prompt | No PT session booked by Day 3 | Prompt sent to book a free fitness assessment with a trainer |
| BR-036 | ONB003 | Day 7 milestone | First week complete | Milestone notification \+ first workout plan reminder sent |
| BR-037 | ONB004 | Day 14 progress check | 2 weeks since activation | Progress check-in prompt \+ encourage body measurement log |
| BR-038 | ONB005 | Day 21 community invite | 3 weeks since activation | Community challenge invitation \+ leaderboard introduction sent |
| BR-039 | ONB006 | Day 30 milestone | First month complete | Month badge awarded \+ personalised progress summary generated and sent |
| BR-040 | ONB007 | Onboarding skip | Member skips a step | Step recorded as skipped; sequence continues; owner funnel report updated |
...
| BR-054 | TRN006 | AI coach activation | Trainer enables AI coaching for a client | AI coach becomes available in member's app for that client; all AI conversations stored and visible to trainer |
| BR-055 | TRN007 | AI coach flag | AI conversation contains injury/pain/medical keywords | Conversation automatically flagged for trainer review; trainer notified; AI does not respond further on that topic |
| BR-056 | TRN008 | Wearable overtraining alert | Client wearable signals high fatigue for 3+ consecutive days | Trainer receives alert; suggestion to review upcoming session intensity |

 

## **8\. Trainer Marketplace Rules**

| BR ID | Source Ref | Rule | Condition | Outcome / Action |
| :---- | :---- | :---- | :---- | :---- |
| BR-057 | MKT001 | Profile visibility | Trainer marketplace is enabled by owner AND trainer has opted in | Trainer profile shown to all members in the app directory |
| BR-058 | MKT002 | Review eligibility | Member has completed at least 1 session with the trainer | Member may submit a rating (1-5 stars) and short review; review moderated by gym before publishing |
| BR-059 | MKT003 | Interest request routing | Member submits PT interest request | Request sent to chosen trainer; trainer has 48h to accept or decline; if no response, owner is notified |
| BR-060 | MKT004 | Virtual session billing | Trainer marks virtual session as completed | Session deducted from client pack and revenue recorded identically to in-person sessions |
| BR-061 | MKT005 | Off-shift virtual sessions | Virtual session conducted outside shift hours | Off-shift rate applied to trainer pay automatically |
...
| BR-067 | FIN006 | Revenue gap alert | Projected weekly revenue (based on scheduled sessions) falls below target by configurable threshold | AI agent flags alert to owner with suggested actions |
...
| BR ID | Source Ref | Rule | Condition | Outcome / Action |
| :---- | :---- | :---- | :---- | :---- |
| BR-070 | ACC001 | Biometric enrolment | New member registered by admin | Member directed to biometric kiosk; face ID or fingerprint captured and stored against member ID |
| BR-071 | ACC002 | Access check sequence | Member scans biometric | System checks in order: (1) biometric match exists, (2) membership is active and not expired, (3) account not suspended — all three must pass for gate to open |
| BR-072 | ACC003 | Override by Gym Owner | Gym Owner (or SuperAdmin) manually overrides access | Override logged with user ID, timestamp, member ID, and reason; treated as a valid check-in |
| BR-073 | ACC004 | Biometric re-enrolment | Existing member's biometric fails consistently | Admin can initiate re-enrolment; old biometric data replaced with new scan |
| BR-074 | ACC005 | Multiple failed scans | Same biometric fails 3 times in a row | Alert sent to Gym Owner; member blocked for 5 minutes; incident logged |
...
| BR ID | Source Ref | Rule | Condition | Outcome / Action |
| :---- | :---- | :---- | :---- | :---- |
| BR-075 | AI-001 | Management accounts schedule | Every Monday morning (or configured day) | AI retrieves prior week's financial and session data; generates P\&L summary and KPI scorecard; delivers to owner dashboard and email |
| BR-076 | AI-002 | Anomaly: revenue drop | Weekly revenue falls more than X% below prior week average (configurable threshold) | AI flags alert to owner with magnitude and possible causes |
| BR-077 | AI-003 | Anomaly: no-show spike | Weekly no-show rate increases more than X% above rolling average | AI flags alert to owner with affected trainers and time slots |
| BR-078 | AI-004 | Anomaly: expense overrun | Any expense category exceeds monthly budget | AI flags alert with overspend amount and budget vs actual comparison |
| BR-079 | AI-005 | Read-only scope | AI agent accesses data | AI agent has read-only access; it cannot write, modify, or delete any financial records |
| BR-080 | AI-006 | NL query scope | Owner asks natural language question via chat | AI retrieves relevant data from GymOS database and responds with summary; does not hallucinate figures not present in data |
| BR-081 | AI-007 | Dynamic pricing recommendation | AI identifies time slot with consistently below-target utilisation | Recommendation surfaced to owner: suggested off-peak discount percentage and estimated utilisation improvement |
| BR-082 | AI-008 | Session pack upsell timing | Client has 2 or fewer sessions remaining in pack | AI triggers personalised upsell prompt to member via app and alerts trainer |
...
# **13\. Process Workflows**

The following end-to-end workflows are transcribed from the GymOS Workflow & Process Flow document (v2.0), which provides the most granular, developer-ready flow detail among the supplied source materials. Each flow uses the format Start → Step → Decision → End and is written so it can be directly converted into a flowchart or BPMN diagram. Equivalent, more narrative descriptions of these same processes also appear in the Project Scope & Workflow Map and the Role Workflows & Feature Guide documents (referenced there as WF-M/T/O series); this section consolidates them into a single canonical set to avoid duplication.

## **Member Flows (MEM-01 – MEM-10)**

### **MEM-01 — Member Registration & Biometric Enrolment  (Actor: Member \+ Admin)**

| Flow | START: New member arrives at gym 1\. Admin opens registration form; enters name, contact, email, DOB, emergency contact, plan selection 2\. Admin selects plan and collects payment DECISION: Payment successful? → YES: proceed to enrolment | NO: show payment error, retry or abandon 3\. Member account created; membership activated; login credentials generated and sent via SMS/email 4\. Member directed to biometric kiosk (face ID or fingerprint) 5\. Biometric data captured and stored securely against member ID DECISION: Enrolment successful? → YES: proceed | NO: re-attempt (max 3), escalate to admin if all fail 6\. Member installs app and logs in AUTO: Onboarding Journey Day 1 triggered automatically END: Registration complete — member active, biometrically enrolled, app-connected |
| :---- | :---- |

 

### **MEM-02 — Daily Gym Access — Biometric Check-in  (Actor: Member)**

| Flow | START: Member approaches gym entrance 1\. Member presents face/fingerprint to kiosk DECISION: Biometric match found? → NO: access denied, front desk alerted, incident logged | YES: continue DECISION: Membership active & not expired? → NO: access denied, renewal prompt sent, front desk alerted | YES: continue DECISION: Account suspended? → YES: access denied, reason logged, admin notified | NO: GATE OPENS 2\. Check-in record created: member ID, timestamp, method, result \= granted AUTO: Retention engine updates attendance score; streak counter checked END: Check-in complete |
| :---- | :---- |
...
### **MEM-07 — 30-Day Onboarding Journey  (Actor: Member \+ System)**

| Flow | TRIGGER: Membership activated AUTO: Day 1 — Welcome message \+ orientation checklist sent DECISION: By Day 3, has member booked a PT session? → NO: Day 3 prompt sent to book a free fitness assessment | YES: step complete AUTO: Day 7 — First-week milestone notification \+ workout plan reminder AUTO: Day 14 — Progress check-in prompt (measurement/photo) AUTO: Day 21 — Community/challenge invitation \+ leaderboard intro AUTO: Day 30 — Month-completion badge \+ personalised progress summary END: Onboarding journey complete — funnel completion/drop-off visible in owner dashboard |
| :---- | :---- |

 

### **MEM-08 — AI Coaching Chat Flow  (Actor: Member \+ AI \+ Trainer)**

| Flow | START: Member opens AI Coach in app DECISION: Has trainer enabled AI coach for this member? → NO: locked screen shown, prompting member to ask trainer | YES: continue 1\. Chat interface loads; plan context and recent history injected 2\. Member sends a message or responds to a daily check-in prompt DECISION: Message contains injury/pain/medical keyword? → YES: AI declines, conversation flagged for trainer | NO: continue 3\. AI generates a response constrained to plan content 4\. Conversation stored and visible to trainer in portal END: Chat session ends (loops back for next message); flagged conversations are resolved by the trainer directly |
| :---- | :---- |

 

### **MEM-09 — Churn Risk Scoring & Re-engagement Funnel  (Actor: System \+ Owner)**

| Flow | TRIGGER: Weekly churn score recalculation (every 7 days) for all active members AUTO: Score (0-100) calculated from weighted signals (check-in recency/trend, no-show rate, expiry proximity, app engagement, onboarding completion) DECISION: Score band? → 0-39 Active (no action) | 40-69 At-Risk (Day1 push, Day3 SMS, Day7 owner alert) | 70-100 Lapsed (immediate owner alert, Day1 offer, Day7 AI message) DECISION: Member checks in during sequence? → YES: sequence halted, status restored to Active | NO: member remains in band until next weekly cycle END: Weekly churn cycle complete — repeats next week |
| :---- | :---- |

 

### **MEM-10 — Gamification — Streak & Challenge Flow  (Actor: Member \+ System)**

| Flow | TRIGGER: Member completes a check-in 1\. System checks weekly visit count against target (default 3/week, configurable) DECISION: Target met? → YES: streak incremented | NO: streak unchanged, nudge sent if at risk of missing target 2\. Milestone check run (1st/10th/100th visit, 30-day streak, final pack session) DECISION: Milestone triggered? → YES: badge awarded \+ notification \+ owner reward if configured | NO: no additional action 3\. Challenge progress updated if member has joined an active challenge; leaderboard rank recalculated DECISION: Challenge goal reached? → YES: completion badge \+ configured reward | NO: progress shown, leaderboard updated END: Gamification cycle complete — repeats on next check-in |
| :---- | :---- |

 

## **Trainer Flows (TRN-01 – TRN-05)**
...
### **TRN-04 — New Client via Trainer Marketplace  (Actor: Member \+ Trainer)**

| Flow | START: Member browses Trainer Marketplace in app 1\. Member views trainer directory (name, photo, specialisations, availability, rating, session count) 2\. Member filters by specialisation/availability (optional) 3\. Member selects a trainer and views full profile 4\. Member submits PT interest request (name \+ goals note); 48-hour response timer starts DECISION: Trainer responds within 48 hours? → NO: request expires, owner notified, member informed | YES: trainer accepts or declines 5\. Trainer accepts and proposes an introductory session (free/discounted, gym-configurable) 6\. Member confirms; session booked in both calendars; standard PT session lifecycle (MEM-04) begins 7\. After session: member may submit an optional review (1-5 stars \+ comment), moderated by owner before publishing END: Marketplace flow complete — trainer-client relationship established |
| :---- | :---- |
...
## **Owner / Admin Flows (OWN-01 – OWN-05)**
...
### **OWN-03 — Re-engagement Campaign Launch  (Actor: Gym Owner)**

| Flow | TRIGGER: Weekly churn recalculation identifies new at-risk/lapsed members 1\. Owner opens retention engine panel (churn score, last visit, recommended action per member) 2\. Owner reviews each flagged member; options: send nudge / send offer / send personal message / view profile DECISION: Automated sequence already running for this member? → YES: owner can supplement with manual message | NO: owner can manually trigger a re-engagement offer 3\. Owner selects an action for the member (e.g., free PT session offer, personalised message, discount code) AUTO: Message/offer delivered via app; action logged in re-engagement log (type, timestamp, sender, actioned flag) DECISION: Member checks in within 14 days? → YES: re-engagement successful, status Active, sequence halted, owner notified | NO: member remains in band, sequence continues or concludes END: Re-engagement flow complete |
| :---- | :---- |

 

### **OWN-04 — AI Agent — Weekly Management Accounts  (Actor: System \+ Owner)**

| Flow | TRIGGER: Scheduled Monday morning run (or configured day) — fully automatic AUTO: AI retrieves prior week's revenue, expense, session, attendance, and churn data AUTO: AI generates P\&L summary (revenue, expenses, payroll, net profit, margin, vs. prior week/rolling average) AUTO: AI generates KPI scorecard (sessions vs target, no-show rate, avg revenue/session, retention rate, new members, pack renewals) AUTO: Anomaly detection runs (revenue drop, no-show spike, expense overrun vs. budget) DECISION: Any anomalies detected? → YES: anomaly alerts generated with magnitude/context | NO: clean report 5\. Report and alerts delivered to owner dashboard; email notification sent DECISION: Owner asks a follow-up question via NL chat? → YES: AI retrieves data and responds in plain language, query logged | NO: report reviewed, end END: Weekly AI management accounts complete |
| :---- | :---- |
...
The screens below are derived functionally from the roles, features, and workflows documented in source material. No visual design (layout, colour scheme, component styling) is specified or implied — only the functional requirements of each screen. Fields marked \[TBD\] indicate that source material does not specify exact validation rules, error copy, or success messaging for that screen; these must be defined collaboratively with the Product Owner and UX designer before implementation.
...
### **SCR-03 — Biometric Enrolment (Kiosk) — Phase 3**

| Purpose | Capture and store a member's face/fingerprint biometric token. Not part of Phase 1 (see Section 5.0). |
| :---- | :---- |
| **Users Who Can Access** | Member (at kiosk), Gym Owner (assist) |
| **Required Fields** | Live camera/fingerprint sensor capture |
| **Buttons / Actions** | Capture scan; Retry (max 3 attempts) |
| **Validation Rules** | Enrolment success/failure detection |
| **Error Messages** | 'Enrolment failed — please try again' \[exact copy TBD\] |
| **Success Messages** | Enrolment confirmed message |
| **Navigation Behaviour** | → App download/login prompt |
| **Permissions / Visibility** | Kiosk-restricted (Phase 3 hardware) |
...
| Purpose | Display the member's PT session attendance log and streak (Phase 1/2: based on PT sessions, per FR-013; Phase 3 may incorporate biometric gym check-ins). |
| :---- | :---- |
| **Users Who Can Access** | Member |
...
### **SCR-13 — Trainer Marketplace Directory**

| Purpose | Browse and filter trainer profiles; submit interest requests. |
| :---- | :---- |
| **Users Who Can Access** | Member |
| **Required Fields** | Specialisation filter, availability filter |
| **Buttons / Actions** | View profile; Submit interest request |
| **Validation Rules** | \[TBD\] |
| **Error Messages** | \[TBD\] |
| **Success Messages** | 'Request sent' confirmation |
| **Navigation Behaviour** | From Home |
| **Permissions / Visibility** | Members only, when marketplace enabled |

 

### **SCR-14 — Gamification Hub (Streaks / Challenges / Leaderboard)**

| Purpose | View streak, join challenges, view/opt into leaderboard, view badges. |
| :---- | :---- |
| **Users Who Can Access** | Member |
| **Required Fields** | Challenge list, leaderboard opt-in toggle |
| **Buttons / Actions** | Join challenge; toggle leaderboard opt-in |
| **Validation Rules** | N/A |
| **Error Messages** | N/A |
| **Success Messages** | 'Milestone achieved' badge display |
| **Navigation Behaviour** | From Home tab bar |
| **Permissions / Visibility** | Member only |
...
### **SCR-27 — AI Agent Panel**

| Purpose | NL Q\&A, weekly management accounts, anomaly alerts, forecasts. |
| :---- | :---- |
| **Users Who Can Access** | Gym Owner, SuperAdmin |
| **Required Fields** | Chat input for NL questions |
| **Buttons / Actions** | Ask a question; review alerts/forecasts |
| **Validation Rules** | N/A |
| **Error Messages** | N/A |
| **Success Messages** | N/A |
| **Navigation Behaviour** | Owner Portal |
| **Permissions / Visibility** | Owner (tier-gated — see Section 5\) |
...
The following preliminary data dictionary identifies the core data entities implied by the functional requirements, business rules, and workflows documented in this SRS. Field lists are representative, not exhaustive, and physical database design (data types, indexing, normalisation) is intentionally left to the Development Team's technical design process.
...
| Churn Score Record | score\_id (PK), member\_id (FK), score (0-100), band (Active/At-Risk/Lapsed), calculated\_date, contributing\_signals | member\_id, score, band, calculated\_date | — | 0 ≤ score ≤ 100 | N:1 with Member (weekly history) | \[TBD\] |
| Re-engagement Log | log\_id (PK), member\_id (FK), type (Automated/Manual), sender, timestamp, actioned (bool), outcome | member\_id, type, timestamp | outcome | — | N:1 with Member | \[TBD\] |
| Challenge | challenge\_id (PK), gym\_id (FK), name, goal\_definition, start\_date, end\_date, reward | name, goal\_definition, start\_date, end\_date | reward | end\_date \> start\_date | 1:N with Challenge Participation | N/A (config/event data) |
...
| AI Conversation | conversation\_id (PK), member\_id (FK), trainer\_id (FK), messages\[\], flagged (bool), flagged\_reason | member\_id, trainer\_id | flagged\_reason | keyword-scan on each inbound message | N:1 with Member; N:1 with Trainer | \[TBD — sensitive if health-adjacent content is discussed, see Section 20\] |
| Wearable Data Record | record\_id (PK), member\_id (FK), provider (Apple Health/Google Fit/Fitbit/Garmin/WHOOP), metric\_type, value, timestamp | member\_id, provider, metric\_type, value, timestamp | — | \[TBD\] | N:1 with Member | \[TBD\] |
...
The table below distinguishes integrations that are explicitly confirmed in source material from those that are named in only one of the two source document sets (FitCore vs. GymOS) and therefore require confirmation.
...
### **Wearable Platforms — Apple Health / Google Fit / Fitbit / Garmin / WHOOP**

| Purpose | Sync member recovery, heart-rate, sleep, and activity data to inform wellness scoring and trainer overtraining alerts. |
| :---- | :---- |
| **Direction of Data Flow** | Inbound (wearable platform → GymOS) |
| **Data Exchanged** | Heart rate, sleep score, HRV, activity/recovery metrics |
| **Trigger** | Member connects a wearable (FR-006); periodic background sync |
| **Authentication / Security** | OAuth per each provider's API \[TBD\] |
| **Expected Response** | Synced metric payloads |
| **Error Handling** | Connection failure shown to member; feature degrades gracefully if unavailable |
| **Dependency Notes** | Phase 3 scope (per Doc 3, Section 1.2 Module table) |
| **Status** | Confirmed as planned (Phase 3), integration mechanics \[TBD\] |

 

### **Biometric Hardware — Face ID / Fingerprint Kiosk (Phase 3\)**

| Purpose | Physical access control device performing biometric capture and matching at gym entrance. Not part of Phase 1 (see Section 5.0) — Phase 1 authentication is username/password only. |
| :---- | :---- |
| **Direction of Data Flow** | Bi-directional (device ↔ GymOS backend for enrolment and match verification) |
| **Data Exchanged** | Biometric template/token (not raw biometric image, per security best practice — \[TBD confirm actual storage approach\]) |
| **Trigger** | Member enrolment; member check-in attempt |
| **Authentication / Security** | Device-level pairing/authentication \[TBD\] |
| **Expected Response** | Match/no-match result |
| **Error Handling** | No-match/failed scan handled per BR-001–BR-005/BR-070–BR-074 (MEM/ACC rules) |
| **Dependency Notes** | FitCore proposal references ZKTeco/Suprema hardware vendors; GymOS documents do not confirm a specific vendor |
| **Status** | \[To Be Confirmed\] — Phase 3; vendor selection and exact device-integration protocol not specified |
...
The system's notification requirements are channel-agnostic in source material except where explicitly stated (push vs. SMS vs. email vs. in-app). Actual channel selection, message copy, and localisation are \[TBD\] beyond what is stated below.
...
| Weekly streak target met/missed | Member | Streak increment confirmation / recovery nudge | BR-GAM001/002 | Weekly |
| Milestone reached (visit count, streak, pack, challenge) | Member | Badge award notification | BR-GAM003-007, 009 | Per milestone |
| Weekly churn recalculation — At-Risk band entered | Member (Day1 push, Day3 SMS); Owner (Day7 dashboard alert) | Re-engagement sequence | BR-CHU002 | Per sequence, weekly recalculation |
| Weekly churn recalculation — Lapsed band entered | Owner (immediate); Member (Day1 offer, Day7 AI message) | Lapsed re-engagement sequence | BR-CHU003 | Per sequence |
| Onboarding journey milestones (Day 1/3/7/14/21/30) | Member | Welcome/checklist/progress/community/badge content per day | BR-ONB001-006 | Per day-based trigger |
| Wearable overtraining signal (3+ days high fatigue) | Trainer | Overtraining alert | BR-TRN008 | Per occurrence |
| Expense exceeds monthly budget in a category | Owner | Budget variance alert | BR-FIN004 | Per occurrence |
| Weekly AI Agent run (Monday, configurable) | Owner | Management accounts summary \+ KPI scorecard (dashboard \+ email) | BR-AI-001 | Weekly |
| AI anomaly detected (revenue drop / no-show spike / expense overrun) | Owner | Anomaly alert with magnitude and context | BR-AI-002/003/004 | Per occurrence |
...
| Member Retention Heatmap & Churn Prediction Report | Visualise attendance frequency colour-coded by churn risk; weekly churn predictions with suggested interventions. | Gym Owner | Churn Score records, Check-in records | \[TBD\] | \[TBD\] | Weekly (aligned to churn recalculation, BR-CHU001) |
| Onboarding Funnel Report | Completion rate and drop-off points across the 30-day onboarding journey. | Gym Owner | Onboarding step completion/skip records | Date range (member cohort) | \[TBD\] | On-demand |
| Gamification Engagement Statistics | Streak participation, challenge completion rates, leaderboard activity. | Gym Owner | Badge, Challenge Participation, Leaderboard records | Date range | \[TBD\] | On-demand |
| Wearable Adoption Rate Report | Proportion of membership with a connected wearable. | Gym Owner | Wearable Data Records | \[TBD\] | \[TBD\] | On-demand (Phase 3\) |
...
| AI Weekly KPI Scorecard | Sessions vs. target, no-show rate, avg revenue/session, retention rate, new members, pack renewals. | Gym Owner | Aggregated operational \+ financial data | N/A (fixed weekly window) | \[TBD\] | Weekly, automated (BR-AI-001) |

# **19\. Non-Functional Requirements**

Measurable targets are provided only where explicitly supported by source material. Where no measurable target exists in source material, the requirement is marked \[TBD\] rather than populated with an invented figure, per instruction.
...
| Biometric no-match | Confirmed: access denied; unrecognised attempt logged; front desk alerted (BR-MEM004). |
...
| Wearable connection/authorisation failure | \[TBD — no explicit error message defined; Assumption: feature is simply unavailable/disconnected\]. |
| AI Agent — insufficient data to answer a natural-language query | Confirmed principle: AI must not hallucinate figures not present in the data (BR-AI-006). \[Exact fallback message/behaviour when data is genuinely unavailable is TBD\]. |
...
Per instruction, this SRS does not invent a technology stack. The items below separate what is explicitly confirmed in source material from what remains an open technical design decision.
...
This is confirmed product direction (not a source-document item, not \[TBD\]): there are exactly two client applications, and they are built in sequence rather than in parallel. This subsection gives the Development Team concrete guidelines for structuring that work.
...
● Neither application is role-exclusive in the sense of a separate codebase per role — each application serves multiple roles through role-gated views within one codebase. "Trainer Portal" and "Owner Dashboard" used elsewhere in this SRS are views within the Web Dashboard, not separate products.

### **22.9.2 API-First Backend — Why This Matters Given the Build Order**

● Design the backend API surface to serve both applications from day one, even though the Mobile App is not built until after the Web Dashboard ships. Retrofitting the API for mobile-specific needs after the fact (push notification tokens, offline-friendly endpoints, QR payloads, lightweight list responses for slower connections) is more expensive than designing for both from the start.

● Concretely: when building any Phase 1 backend endpoint that the source material describes as a Mobile App function (e.g., PT session QR start/end — FR-015/FR-016; hire-a-trainer — FR-082; workout/meal plan viewing — FR-021/FR-022), implement and test that endpoint fully even though the Web Dashboard may not have a UI for it. The Web Dashboard can expose a simple admin/testing view for these mobile-only functions (e.g., a 'mark session complete' button, already specified as FR-037) purely to allow backend verification before the Mobile App exists.

● Authentication (FR-002, username/password) should be implemented as a shared API concern (e.g., a single auth service/endpoint set) consumed identically by both applications, so no rework is needed when the Mobile App is added.

### **22.9.3 Recommended Build Order Within Phase 1**

1\. Backend & data model (Section 15\) — built to serve both clients from the start, per 22.9.2.

2\. Web Dashboard: Gym Owner / SuperAdmin functions — gym configuration (FR-049–FR-055), member management (FR-057/FR-058), trainer management (FR-060–FR-062), financial management (FR-066–FR-070), dashboard/analytics (FR-071).
...
*This order is a recommendation for sequencing Phase 1 work, not a rigid mandate — the Development Team should adjust based on actual technical dependencies once implementation begins.*

### **22.9.4 Known Consequence of Building Web-First**

● Any Phase 1 requirement that is Mobile-only by nature (QR-based session start/end performed by the member, hire-a-trainer performed by the member) cannot be end-to-end tested by an actual member until the Mobile App exists. Recommend the Web Dashboard include minimal internal testing affordances (e.g., a way to simulate a member action) so backend logic can be validated ahead of Mobile App delivery — see 22.9.2.

● Trainer-side workflows split across both apps (e.g., a trainer accepts a hire request which might arrive via Web Dashboard notification, but the actual session happens via Mobile App QR scan) should be designed so the trainer's account and data are identical across both apps from day one, to avoid a migration/sync problem when the Mobile App ships.

 

*Per instruction \#10, this SRS deliberately does not invent a technology stack. All technical requirements above marked \[TBD\] should be defined collaboratively by the Product Owner and the Development/IT Team as an early-phase technical design activity, informed by the functional and data requirements documented in Sections 9 and 15\.*

# **23\. Performance Requirements**

## **23.1 Known, Confirmed Values (from Business Rules & Workflows)**
...
| Biometric lockout after repeated failures (Phase 3\) | 3 consecutive failed scans → 5-minute lockout (BR-070–074, Access Control Rules) |
...
High-level, measurable acceptance criteria for the overall system and its major modules. Module-level and requirement-level acceptance criteria also appear throughout Sections 9-11.
...
● 100% of biometric check-in attempts result in a logged outcome (granted or denied with reason).

● A member with an expired or suspended membership cannot gain gate access without a logged manual override.

● 3 consecutive failed scans reliably trigger a 5-minute lockout and an incident log entry.
...
● Churn scores are recalculated for all active members on a 7-day cycle without manual triggering.

● Band transitions (Active/At-Risk/Lapsed) correctly trigger the corresponding notification sequence per BR-CHU002/003.
...
The following assumptions have been made where source material was silent or ambiguous. These are assumptions, not confirmed requirements, and should be validated with the Product Owner.
...
● Wearable platform providers (Apple Health, Google Fit, Fitbit, Garmin, WHOOP) — Phase 3 API access.

● Biometric hardware vendor — selection, procurement, and integration protocol (currently unconfirmed).
...
● Biometric kiosk hardware provisioning per gym location.
...
| R-02 | Biometric hardware failure at the entrance kiosk (Phase 3 risk — not applicable until biometric access control is built, see Section 5.0) | Low | High | On-site spare units; manual Gym Owner override (BR-072/ACC003) already provides a functional fallback path | Gym Owner / Hardware Vendor | Open — hardware vendor not yet selected; not urgent until Phase 3 |
...
| R-06 | Biometric and health-adjacent data (AI Coach conversations, medical notes) handled without a confirmed data-protection/compliance policy | Medium | High | Confirm applicable data-protection law (e.g., Sri Lanka's Personal Data Protection Act) and define a formal data classification/handling policy before these modules are built | Product Owner / Legal | Open — see Section 20 and Open Questions §31.9 |
| R-07 | AI Agent (Phase 3\) provides an incorrect or fabricated figure to an Owner, undermining trust in financial reporting | Low (given explicit read-only/no-hallucination rule) | High | BR-AI-005 (read-only) and BR-AI-006 (no hallucination) are already specified as hard constraints; must be enforced and tested at implementation | Development/IT Team / QA | Open — pending Phase 3 implementation and test design |
...
# **31\. Open Questions / Items Requiring Confirmation**

This section is the single most important reference point before development begins on any ambiguous area. Each item below represents a genuine gap or conflict between source documents, not a stylistic preference.
...
| Q-06 | Which biometric modality is the confirmed Phase 3 target: fingerprint only, face ID only, or both simultaneously? | FR-010, Section 16 (Biometric Hardware) | The FitCore proposal describes a fingerprint-only kiosk; the GymOS Role Function List and workflow documents consistently describe 'face ID or fingerprint' as parallel options. No longer MVP-blocking since biometrics moved to Phase 3 (Section 5.0), but still needed ahead of Phase 3 planning. | Founder / Product Owner | Open |
...
| Q-11 | What biometric hardware vendor is confirmed for production kiosks? | Section 16 | Only the FitCore business proposal names vendors (ZKTeco / Suprema); GymOS technical documents are vendor-agnostic. | Founder / Product Owner | Open |
...
| Q-14 | Now that biometric gym-door check-in is deferred to Phase 3, should Phase 1/2 attendance streaks (BR-GAM001/002) be based on PT session attendance only, or should some other Phase-1 attendance signal be defined? | FR-013, Section 12 (Gamification Rules) | The source business rules define the weekly streak target as "check-ins," which originally meant biometric gym-door check-ins. This SRS defaults to PT-session-based streaks for Phase 1/2 as the closest available signal, but this is an assumption, not a confirmed decision. | Founder / Product Owner | Open — new in this revision |
...
The matrix below connects each primary/secondary Business Objective (Section 4\) to its representative Functional Requirements, User Stories, Acceptance Criteria, and planned Test Coverage. Full requirement-level detail is in Sections 9-11 and 24-25; this view exists to demonstrate that every objective is covered end-to-end.
...
| OBJ-05 | Member retention (churn/onboarding) | FR-009, FR-059, FR-072 | US-021 | Retention / Churn Engine AC (Sec. 24\) | UC-09; Regression Testing |
| OBJ-06 | Gamification / engagement (Phase 2\) | FR-031 – FR-033 | US-009 | (Engagement covered under Overall System AC) | UC-14; Unit Testing (streak/milestone logic) |
| OBJ-07 | Public Trainer Marketplace (Phase 2\) | FR-029, FR-043, FR-047, FR-048 | US-010, US-016 | (Marketplace covered under Overall System AC) | UC-08; System Testing |
| OBJ-08 | AI Agent business intelligence (Phase 3\) | FR-074, FR-075, FR-076 | US-022 | (AI Agent covered under Overall System AC) | UC-13; Security Testing (BR-AI-005/006 enforcement); UAT |
| OBJ-09 | Wearable integration (Phase 3\) | FR-006, FR-025, FR-040 | (No dedicated story authored — Phase 3\) | (Not yet defined — Phase 3\) | Integration Testing (Phase 3, pending provider confirmation) |
| OBJ-10 | Biometric gym-door access control (Phase 3\) | FR-010, FR-011, FR-012, FR-013 | US-002, US-024 | Access Control module AC (Sec. 24\) | System Testing (UC-02); Unit Testing (BR-MEM/ACC rules) — Phase 3 |
...
## **Appendix A — Source Documents Consolidated Into This SRS**
...
● GymOS | Role Workflows & Feature Guide | v2.0 (plain-language reference for Member, Trainer, Owner/Admin).

● GymOS | Workflow & Process Flow Document | v2.0 (developer-facing, flow-only reference, MEM/TRN/OWN series).

## **Appendix B — Notation Conventions Used in This SRS**
...
● \[Assumption\] — A reasonable inference made by the author of this SRS to allow the document to proceed; must be validated.

● \[Recommendation\] — A suggested approach (often carried from the FitCore business proposal's risk analysis) that is not yet an agreed GymOS requirement.

● Unmarked statements — Treated as confirmed, directly sourced from the supplied documents.

# **35\. Requirements Completeness Review**

## **35.1 Missing Information**

● No confirmed technology stack (frontend, backend, database, hosting) exists for GymOS (Section 22).

● No confirmed default values for trainer pay rates, cancellation window, or several other explicitly-flagged-as-configurable thresholds (Section 23.1 notes which values are illustrative-only).

● No confirmed SMS gateway, email service provider, or biometric hardware vendor (Section 16).

● No performance/capacity targets (concurrent users, transaction volume, data volume) exist for technical capacity planning (Section 23.2).

● No confirmed compliance regime, backup/disaster-recovery strategy, or uptime/SLA target (Sections 19-20).

## **35.2 Ambiguous Requirements**

● Whether SuperAdmin actions taken on behalf of a Gym Owner (e.g., support/troubleshooting) should be distinguished in the audit log from the Gym Owner's own actions (Section 7, Section 20).

● The exact biometric modality (fingerprint-only vs. face-and-fingerprint) intended for the confirmed MVP (Open Questions Q-06).

● Whether 'on-time' (outside-policy-window) cancellations incur any deduction at all — only the 'late' case is explicitly documented (Section 27, Assumptions).

## **35.3 Conflicting Requirements**

● FitCore vs. GymOS: different project/brand names, different subscription tier names and feature-to-tier mappings, different payment gateway pairings (Stripe+PayHere vs. PayHere+iPay), presence vs. absence of WhatsApp notification as a headline feature, and different development-timeline claims (12-month 3-phase plan vs. 3-month MVP). All are catalogued in Section 31 and must be resolved before the affected modules are built.

## **35.4 Requirements Needing Confirmation**

See Section 31 (Open Questions) in full — 15 distinct items require Founder/Product Owner (and in some cases Development Team) input before the corresponding requirements can be treated as final.

## **35.5 Recommended Next Steps Before Development Begins**

● 1\. Hold a scope-reconciliation session with the Founder/Product Owner to resolve the FitCore-vs-GymOS conflicts in Section 31 (Q-01, Q-02, Q-03, Q-04, Q-09) and formally re-baseline this SRS if scope changes as a result.

● 2\. Confirm default configuration values (pay rates, cancellation window, streak targets) with the Product Owner — these are explicitly flagged in source material as pending Owner-side configuration, not engineering defaults.

● 3\. Run a technical design workshop (Product Owner \+ Development/IT Team) to select the technology stack, hosting model, and API approach (Section 22), informed by the data model in Section 15 and functional scope in Section 9\.

● 4\. Confirm third-party vendor selections: payment gateway(s), SMS/email providers, biometric hardware vendor, and (if in scope) wearable providers (Section 16).

● 5\. Define measurable non-functional targets (performance, availability, backup/DR) with the Product Owner and Development Team before Phase 1 technical design is finalised (Sections 19, 23).

● 6\. Confirm applicable data-protection/compliance obligations with legal counsel, particularly given the system's handling of biometric and health-adjacent data (Section 20, Open Questions Q-07).

● 7\. Once the above are resolved, update this SRS to v1.1, converting resolved \[TBD\]/\[To Be Confirmed\] items into confirmed requirements, and use the resulting document as the formal development baseline.
